...
and production deployment/release approval remain required.
...
**Storage**: SQL Server 2022 Developer compatibility 160 with Code First
migrations; Docker Development persists until guarded reset, and
Testcontainers creates/disposes isolated per-run Testing databases; all seed
data is synthetic
...
- Define an explicit non-production bootstrap contract: migrate first, then
  idempotently seed a versioned deterministic logical fixture; allow reset only
  through a separately invoked Development/Testing guard and never through a
  schema migration or production startup path.
- Require SQL Server 2022 Developer compatibility level 160 for the demo,
  Docker for persistent-until-reset Development, and Testcontainers for
  per-run Testing creation/disposal. Reject real institutional/student data;
  this makes no production edition/topology decision.
...
- Keep the AASTMT Operations deployment-window duration In Review and block
  production deployment/release readiness until a numeric window is approved;
  rehearsal target is 80% of it.
...
2. Verify Ahmed ELbamby's 2026-07-13 approval in the Data Lead review
   perspective before implementation; retain later production migration and
   release approvals.
3. Test the ownership/invariant/lifecycle schemas and the isolated
   Development/per-run Testing bootstrap, SQL version/compatibility,
   Docker/Testcontainers provisioners, per-run disposal, idempotent seed,
   explicit reset, synthetic-only, credential-redaction, Git-ignore, and
   seven-day cleanup guards before publishing them.
...
- NFR-4: Sensitive fields MUST be minimized and excluded from unsafe logs;
  local credential artifacts, logs, and exports are Git-ignored and removed
  within seven days.

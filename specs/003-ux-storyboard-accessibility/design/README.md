...
**Contract version:** `frontend-design-index/1.0`
...
| API dependency and authority owner | `.specify/page-api-manifest.json` |
...
- `pages/*.md`: one immutable approved design record per route ID.

## Required route trace row

Each route Page Design Record is the trace row. It must link the route to:

1. its owner SPEC and FR/AC references from the route manifest;

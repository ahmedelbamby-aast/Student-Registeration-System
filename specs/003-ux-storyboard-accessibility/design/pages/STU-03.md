...
| `STU-03-pending-approval-v2` (pending approval/held) | success | pending approval status, held-capacity effect, expiry, and no selectable success claim | confirmed enrollment, hidden held capacity, or duplicate request | Review the pending request | `STU-03-COMP-STATE-SUCCESS` | `STU-03-E2E-PRIMARY` |
| `STU-03-approved-v2` (approved) | success | authoritative approval result, reference, and current capacity | client-created approval or guaranteed registration | Continue through normal server revalidation | `STU-03-COMP-STATE-SUCCESS` | `STU-03-E2E-PRIMARY` |
| `STU-03-rejected-v2` (rejected) | validation-error | rejection reason, immutable reference, and safe alternative action | enabled enrollment action or hidden reason | Return to discovery or choose an alternative | `STU-03-COMP-STATE-VALIDATION-ERROR` | `STU-03-E2E-FAILURE` |
| `STU-03-expired-released-v2` (expired/released) | stale | expired request and released capacity with refresh guidance | retained seat claim or stale approved status | Refresh authoritative subject state | `STU-03-COMP-STATE-STALE` | `STU-03-E2E-FAILURE` |

...
- [x] T037 [EC-2] Exercise EC-2 with fault/boundary injection in tests/StudentRegistration.IntegrationTests/Specs/Spec018/EdgeCases/EC-2Tests.cs and assert: One app instance fails -> load balancer removes it; other instance continues with shared auth keys/database.
- [x] T038 [EC-3] Exercise EC-3 with fault/boundary injection in tests/StudentRegistration.IntegrationTests/Specs/Spec018/EdgeCases/EC-3Tests.cs and assert: SQL unavailable -> fail safely, no partial result; health turns unhealthy and user gets reference ID.
- [x] T039 [EC-4] Exercise EC-4 with fault/boundary injection in tests/StudentRegistration.IntegrationTests/Specs/Spec018/EdgeCases/EC-4Tests.cs and assert that compatibility mismatch or failure to positively validate both Development/Testing environment and connection target stops bootstrap/reset before any seed or destructive mutation.
- [x] T040 [EC-5] Exercise EC-5 with fault/boundary injection in tests/StudentRegistration.IntegrationTests/Specs/Spec018/EdgeCases/EC-5Tests.cs and assert: Load targets prove unrealistic -> rebaseline by approved spec change, never silently relax correctness.
...
- [x] T073 [TRACE] Generate the completed FR/NFR/SC/AC/EC/route-to-test evidence matrix at docs/release-evidence/SPEC-018-traceability.md and reject release if any row lacks passing evidence.
...
T001-T066 and T068-T073 are complete. T067 and T074 remain evidence-gated by the unperformed manual keyboard/NVDA journeys and unsigned UX/QA approval; skipped or pending evidence cannot authorize release.

...
namespace StudentRegistration.Infrastructure.SqlServer.Migrations
{
    ...
    [Migration("20260720185102_Spec014ApprovalSeatHoldsAndFirstTermAutomation")]
    partial class Spec014ApprovalSeatHoldsAndFirstTermAutomation
    {
        /// <inheritdoc />
...
#pragma warning disable 612, 618
            ...
            modelBuilder.Entity("StudentRegistration.StaffAdministration.Domain.ExportJob", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("JobId");
                    ...
                    b.HasIndex("OwnerId", "ScopeHash", "ClientRequestId")
                        .IsUnique();

                    b.HasIndex("State", "LeaseExpiresAtUtc", "AttemptCount", "Id");

                    b.ToTable("ExportJobs", "administration", t =>
                        {
                            t.HasCheckConstraint("CK_ExportJobs_AttemptCount", "[AttemptCount] >= 0 AND [AttemptCount] <= 3");

                            t.HasCheckConstraint("CK_ExportJobs_LeaseShape", "(([State] = 'running' AND [LeaseOwnerId] IS NOT NULL AND [LeaseExpiresAtUtc] IS NOT NULL AND [AttemptCount] >= 1) OR ([State] <> 'running' AND [LeaseOwnerId] IS NULL AND [LeaseExpiresAtUtc] IS NULL))");

                            t.HasCheckConstraint("CK_ExportJobs_ResultShape", "(([State] IN ('pending', 'running') AND [ArtifactId] IS NULL AND [CompletedAtUtc] IS NULL AND [ExpiresAtUtc] IS NULL AND [FailureCode] IS NULL) OR ([State] IN ('complete', 'expired') AND [ArtifactId] IS NOT NULL AND [CompletedAtUtc] IS NOT NULL AND [ExpiresAtUtc] IS NOT NULL AND [ExpiresAtUtc] > [CompletedAtUtc] AND [FailureCode] IS NULL) OR ([State] = 'failed' AND [ArtifactId] IS NULL AND [CompletedAtUtc] IS NOT NULL AND [ExpiresAtUtc] IS NULL AND [FailureCode] IS NOT NULL))");

                            t.HasCheckConstraint("CK_ExportJobs_State", "[State] IN ('pending', 'running', 'complete', 'failed', 'expired')");
                        });
                });
